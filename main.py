...
def get_knapsack() -> tuple:
    ...
    with open(f'test/p01_c.txt', 'r') as file:
        capacity = [int(line.rstrip()) for line in file][0]

    with open(f'test/p01_p.txt', 'r') as file:
        profits = [int(line.rstrip()) for line in file]

    with open(f'test/p01_w.txt', 'r') as file:
...
capacity, profits, weights = get_knapsack()

start_time = time.time()
a = Knapsack(weights, profits, capacity)
